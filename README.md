...
Each node controls 4 advanced monitors (6x7 each) on:
- front
- left
- back
- right
...
### Node computer
- Advanced monitor on `front`
- Advanced monitor on `left`
- Advanced monitor on `back`
- Advanced monitor on `right`
...
The installer asks for stack index (1..20), writes `node_config.lua`, downloads runtime files, and creates `/startup` to run `node.lua` automatically.
